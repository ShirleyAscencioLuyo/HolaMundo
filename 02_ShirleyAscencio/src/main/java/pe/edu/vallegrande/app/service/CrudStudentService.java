...
public class CrudStudentService implements CrudServiceSpec<Student>, RowMapper<Student> {
	...
	private final String SQL_SELECT_BASE = "SELECT student_id, names, lastname, email, cellphone, career, semester, document_type, document_number, active FROM student";
	private final String SQL_INSERT = "INSERT INTO student (names, lastname, email,  cellphone, career, semester, document_type, document_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
	private final String SQL_UPDATE = "UPDATE student SET names=?, lastname=?, email=?, cellphone=?, career=?, semester=?, document_type=?, document_number=? WHERE student_id=?";
	...
	public List<Student> get(Student bean) {
		...
		cellphone = "%" + UtilService.setStringVacio(bean.getCellphone()) + "%";
		...
		try {
			...
			// La consulta
			sql = SQL_SELECT_BASE
					+ " WHERE names LIKE ? AND lastname LIKE ? AND email LIKE ? AND cellphone LIKE ? AND career LIKE ? AND semester LIKE ? AND document_type LIKE ? AND document_number LIKE ?";
			pstm = cn.prepareStatement(sql);
